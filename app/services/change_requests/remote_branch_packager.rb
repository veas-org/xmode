...
module ChangeRequests
  class RemoteBranchPackager
    def self.call(change_request, step, repository:, token:)
      new(change_request, step, repository: repository, token: token).call
    end

    def initialize(change_request, step, repository:, token:)
    ...
    def call
      ...
      @run.append_log("Remote review branch pushed: #{@change_request.branch_name}", step: @step)
      package
    end
      ...
      if token_push_url?
        with_askpass do |env|
          system!(
            ...
            "push",
            push_url,
            "#{@change_request.branch_name}:#{@change_request.branch_name}",
            chdir: sandbox_root.to_s
          )
        end
      else
    ...
    def token_push_url?
      push_url.start_with?("https://") && @token.present?
    end
    ...
    def with_askpass
      script = Tempfile.new("xmode-git-askpass")
      script.write(askpass_script)
      script.close
      FileUtils.chmod(0o700, script.path)
      yield(
        {
          "GIT_ASKPASS" => script.path,
          "GIT_TERMINAL_PROMPT" => "0",
          "XMODE_GIT_USERNAME" => git_username,
          "XMODE_GIT_TOKEN" => @token.to_s
        }
      )
    ensure
      script&.close!
    ...
    def git_username
      @repository.provider == "gitlab" ? "oauth2" : "x-access-token"
    end
    ...
    def system!(*command, chdir:)
      ...
      display = command.first.is_a?(Hash) ? command.drop(1) : command
      raise "Command failed: #{display.join(' ')}"
    end
  end
end
