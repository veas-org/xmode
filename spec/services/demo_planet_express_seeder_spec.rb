...
RSpec.describe Demo::PlanetExpressSeeder do
  it "seeds an idempotent Bender demo workspace" do
    ...
    expect(workspace.agent_definitions.pluck(:key)).to include("planning-agent", "implementation-agent", "cloud-sandbox-agent", "verification-agent")
    expect(workspace.agent_swarm_definitions.pluck(:key)).to include("implementation-swarm", "cloud-sandbox-swarm", "maintenance-swarm")
    ...
    expect(workspace.action_definitions.find_by!(key: "plan-story").agent_definition.key).to eq("planning-agent")
    expect(workspace.action_definitions.find_by!(key: "cloud-rails-code").agent_definition.key).to eq("cloud-sandbox-agent")
